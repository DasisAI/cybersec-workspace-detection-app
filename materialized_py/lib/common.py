...
if city_db_path:
    try:
        ...
        print(f"? ï¸ GeoIP City database found but failed to load: {e}")
...
if asn_db_path:
    try:
        ...
        print(f"? ï¸ GeoIP ASN database found but failed to load: {e}")

# If neither is configured, note that it's disabled (not an error)
if not city_db_path and not asn_db_path:
    print("?¹ï¸ GeoIP enrichment disabled (not configured)")

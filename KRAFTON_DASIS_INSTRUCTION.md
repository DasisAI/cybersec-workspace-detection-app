...
*   **`detections/`**: 본 프로젝트에서 주로 관리하는 **Findings Rules**가 포함되어 있습니다.
    *   `detections/` 내의 원본 룰들은 추후 dasis의 `materialized_py/`로 변환되어 실제 탐지에 사용됩니다.
    *   각 폴더는 룰 그룹을 의미하며, 각 폴더 안에는 룰을 정의하는 파이썬 파일들이 포함되어 있습니다.
        - **Behavioral**: 30일 기준 데이터를 분석합니다.
        - **Binary**: 24시간 기준 데이터를 분석합니다.
        - **Custom**: 사용자가 직접 정의한 예시 룰 그룹입니다. 다른 이름의 폴더를 생성하여 새로운 룰 그룹을 추가할 수도 있습니다.
